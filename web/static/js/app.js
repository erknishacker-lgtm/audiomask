      ...
      <div class="auth-wrap fade-in">
        <div class="auth-card">
          <div class="auth-brand">
            <img src="/assets/logo_transparent.png" alt="zGhost Wave" width="32" height="32" />
            <span>zGhost Wave</span>
          </div>
          <h1 class="auth-heading">${tab === "login" ? "Entre na sua conta" : "Crie sua conta"}</h1>
          <p class="auth-sub">${tab === "login" ? "Acesse seu painel zGhost Wave." : "Comece a proteger seus criativos."}</p>
          <form id="authForm" class="auth-form">
            ${tab === "register" ? `<div class="field"><label for="auth-name">Nome</label><input id="auth-name" name="name" required autocomplete="name" placeholder="Seu nome" /></div>` : ""}
            <div class="field"><label for="auth-email">E-mail</label><input id="auth-email" name="email" type="email" required autocomplete="email" placeholder="seu@email.com" /></div>
            <div class="field"><label for="auth-pass">Senha</label><input id="auth-pass" name="password" type="password" required minlength="6" autocomplete="${tab === "login" ? "current-password" : "new-password"}" placeholder="••••••••" /></div>
            <button class="btn btn-primary btn-block auth-submit" type="submit">${tab === "login" ? "Entrar" : "Criar conta"}</button>
          </form>
          <p class="auth-switch">
            ${tab === "login"
              ? `Não tem conta? <button type="button" class="auth-link" data-tab="register">Cadastre-se</button>`
              : `Já tem conta? <button type="button" class="auth-link" data-tab="login">Entrar</button>`}
          </p>
        </div>
      </div>`;
    bindLang();
    ...
    getContent().innerHTML = `
      <div class="shell fade-in">
        <div class="page-head">
          ...
          <p class="lead">Sem jargão. Pense no zGhost Wave como um <strong>envelope com duas cartas</strong> no mesmo pacote.</p>
        </div>
        ...
        <div class="tutorial-grid">
          ...
          <div class="tutorial-card">
            ...
            <p>Nenhuma ferramenta garante 100% para sempre. As IAs mudam. O zGhost Wave maximiza a chance: <strong>humano ouve black; máquina tende a white</strong>. Sempre teste a legenda na plataforma antes de escalar spend.</p>
          </div>
        </div>
